...
lost_medicaid <- data_2019 %>% 
  select(SSUID,MONTHCODE,RHLTHMTH,RPUBTYPE2) %>% 
  #creating text "lag" field
  arrange(SSUID,MONTHCODE) %>% 
    mutate(
      #recode health insurance bulean to be "no_insurance"
      
      #recode medicaid bulean to be "medicaid"
      
      #create field of "medicaid_last_month" as a text lag field
      
      #create variable subtracting "no_insurance" & "medicaid" so that values that = 2 mean someone experienced a lag in medicaid
      
      #create "lost_medicaid" variable with last field

...
### Adding the variable "lost_medicaid_coverage" to the January filtered dataset
```{r}
...
lost_medicaid <- lost_medicaid_detail %>% 
  ...
  group_by(unique_id) %>% 
    mutate(
      ...
      medicaid_last_month = lag(medicaid,default = 0),
   ...
   #transform data where each row represents a unique participant
    pivot_wider(
      id_cols = unique_id, 
      names_from = month,
  ...
  #remove \January column, since we are not looking at people who lost coverage in January
    ...
    #Create new field that adds together each month, where values >0 means someone lost medicaid during the year
      mutate(
        medicaid_lag = sum(February, March, April, May, June, July, August, September, October, November, December),
        medicaid_lag = case_when(
          medicaid_lag >= 1 ~ 1,
          medicaid_lag < 1 ~ 0)
      )

#Add in outcome variable to main dataset
...
### correlation (Alicia)
  -   did a review of the top correlated variables 

...
```{r}
#processing outcome variable - individuals covered by insurance that experienced a gap in coverage during 2019
...
recipe <- recipe(insurance_gap ~., training) %>%
  update_role(unique_id,state_name,SSUID,PNUM,ENJ_LAYOTYP,insurance_gap,
              EJB4_BSLRYB,TJB3_GAMT1,TSUR10AMT, new_role = "id") %>%
  step_rm(has_role("id")) %>%

...
#filtering individuals will medicaid in January
...
#processing outcome variable - individuals covered by medicaid that experienced a gap in coverage during 2029
lost_medicaid_detail <- data_2019 %>% 
  filter(SPANEL == 2019) %>% 
...
lost_medicaid <- lost_medicaid_detaill %>% 
  #creating text "lag" field
  arrange(unique_id,MONTHCODE) %>% 
    mutate(
      ...
      #create field of "medicaid_last_month" as a lag field
      medicaid_last_month = lag(medicaid, n = 1),
      
      #create variable adding "no_insurance" & "medicaid" so that values that = 2 mean someone experienced a lag in medicaid
  ...
  #transform data into setting where each row represents a unique participant
    pivot_wider(
      id_cols = unique_id, 
      names_from = month,
      values_from = lost_medicaid) %>% 
  #Create new field that adds together each month, where values >0 means someone lost medicaid
      mutate(
        medicaid_lag = sum(January,February,March,April,May,
                           June,July,August,September,October,
                           November,December))
      
      #create df with unique individuals and thier "lost_coverage" variables for each month

      
      #create new variable summing all of the values, if anyone is > 0, it means they experienced a lag at some point of the year
    
    
#left join January filtered data to the lost medicaid so that outcome variable for only those that started with medicaid have the lost_medicaid variable on them

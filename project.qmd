...
We created the outcome variable insurance_gap, which indicates if the respondent who had insurance in January of 2018 lost coverage at any point over the course of a year. Most statistics published focus on individuals who experienced an insurance gap across the course of a year, and therefore it was difficult to find a clear point of comparison to understand if the distribution of the outcome variable as calculated was well-aligned with what we would expect of the broader population.

In looking at the raw metrics generated by our sample, we found that only 696 out of over 63,000 individuals were estimated to have lost insurance, representing around 1% of the sample population.
...
Given how low this metric appeared, we decided to conduct a nonparametric binomial test. With the null hypothesis that the true range of proportions of those that lost insurance over the course of the year in our sample would be equal to that of the broader population.The alternative would be that the proportion of those within our sample would be different than that expected in the broader populations.

Unfortunately, this test depends on an in-depth knowledge of insurance gap rates, and there were not well published statistics on how many people who had insurance in January of 2018 lost it over the course of the year (only those that were uninsured at a given period of time, including in January). Therefore, we had to make assumptions when designing our binomial test.

To do this, we found that the [The Center on Budget and Policy Priorities](https://www.cbpp.org/research/health/closing-medicaid-coverage-gap-would-help-diverse-group-and-narrow-racial#_ftn6) reported that around 13% of middle-aged individuals that had Medicaid in January had experienced some kind of a gap in coverage throughout the year. We would expect that this percentage be lower when considering the broader US population, which would include individuals that are NOT covered by Medicaid and those that might be less likely to lose insurance (such as those of high socioeconomic status within high salary jobs). We decided to make an assumption, based on limited data, that the true probability of losing insurance would be 5%.
...
The results of our binomial test would be to reject the null hypothesis at a 90% confidence level. Under an assumption of a robust sample, this could indicate that the true proportion of someone to experience a loss of insurance throughout the year would be between 1% and 1.1%. For our own interpretation, however, it would indicate that our outcome variable within our sample would not seem to align with the expected percentage of individuals that would experience coverage loss based on other available data. Based on subject matter knowledge, we would have anticipated the estimated proportion of those to lose insurance to be higher.

Given time constraints for our analysis, we used this information to guide our model creation process in several ways. First, the Census recommends the utilization of weights when calculating test statistics. It is possible that the raw, unweighted sample, is underrepresenting demographic groups that would be most likely to lost insurance. For example, there is ample literature that discusses disparities in health coverage between white and black racial groups. If black americans were undersample, it could contribute to a lower than expected insurance loss rate.

Ultimately, we will balance this dynamic in a few ways: the first is that we will incorporate weights into our model pre-processing procedures to mitigate for systematic undersampling. Additionally, given that such a low proportion of individuals would be predicted to be low, we will look across various error metrics for our model to ensure and the model has a strong balance of recall, precision and accuracy. This can ensure that, predicting no one to lose insurance, for example, will not be hidden beneath extremely high recall rates.

### Examining Variable Related to Health Insurance
...
#data_2018_final_rf_svy <- svydesign(
  #ids = ~unique_id,
  #weights = ~REPWGT0,
...
recipe <- recipe(insurance_gap ~., training) %>%
  update_role(unique_id,state_name,SSUID.x, SSUID.y, PNUM.x, PNUM.y,
              ENJ_LAYOTYP,
...
final_model_fit <- 
  final_workflow %>% 
  fit(data = training)#, weights = REPWGT0) 

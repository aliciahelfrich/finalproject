...
---
#Predicting Uninsurance Status Using Supervised Machine Learning
...
data_2019 <- left_join(data_2019, rw_2018, by = c("SSUID","PNUM","MONTHCODE"))
#Merge primary data and replicate weights on SSUID, PNUM, MONTHCODE, SPANEL, and SWAVE

##apply weights to data (might need to do this later to avoid a ton of data)
data_2018 <- inner_join(pu, rw, by = c("SSUID","PNUM","MONTHCODE")) sipp.svy = svrepdesign( data = data, weights = ~WPFINWGT, repweights = "REPWGT[1-9]+", type = "Fay", rho = 0.5 ) svymean( ~TPTOTINC , sipp.svy , na.rm = TRUE )
...
The above visualization shows us the distribution of food security in our sample. Food security can be an important predictor of uninsurance. One study found that compared to people who are food secure, individuals with food insecurity "postpone needed medical care and medications more often, use more emergency and inpatient care, and have higher care expenditures by as much as 121%."@Sonik
...
We are dropping variables with complete missingness to prep for our model.

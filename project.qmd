---
title: "Predicting Insurance Loss Using Supervised Machine Learning"
...
The SIPP doesn't have an explicit variable that indicates if someone lost insurance throughout the calendar year. Therefore, we needed to use available data to generate a variable for "insurance_gap", which acted as our outcome variable. The insurance_gap variable is a boolean field, where "1" means that they had insurance the prior month and then didn't the following. A "0" would signify that someone either didn't have insurance for those two consecutive months, did have insurance, or gained insurance after not having it.
...
Many individuals are insured with Medicaid, which can vary in terms of eligibility requirements across states and can change substantially over the course of the year. For this reason, we believed that the particular states' frequency with which eligibility checks are made, and other local Medicaid policy specifications, could be a useful predictor of insurance loss. (For example, three states allow for 12-month continuous eligibility, while others conduct periodic eligibility checks before the standard annual renewal date. To account for some of these variations, we have created several indicator variables.[^7][^8]. We compiled a small data set that summarizes these characteristics and joined them to our main SIPP data based on the respondent's state of residence at the beginning of the year.
...
The null hypothesis for our binomial test was that the proportion of individuals within our sample that lost insurance throughout the course of the year is equal to the rate of insurance loss for the broader population. The alternative was that these two proportions would not be equal. In order to properly conduct this test, we needed to understand the true rate of insurance loss for the US population over the course of a year to serve as our p value. Unfortunately, while there is much literature on how many individuals are uninsured within the course of a year, there are not readily available numbers on who many people experience the new loss of insurance across a year. Therefore, we needed to make several assumptions based on the information available.

We were able to find that 13% of middle-aged individuals that had Medicaid in January had experienced some kind of a gap in coverage throughout the year.[^9] Using this as an anchor, we would expect that this proportion would be lower for the general population. Those on private insurance, which make up most cases (as we show below), are more likely to be in sector that is at lower risk of turnover(and thus loss of private insurance), or be more likely to have resources to purchase personal insurance in the event of losing their job. We decided to assume, based on limited data, that the true probability of losing insurance for our population of interest would be 10%.
...
These results raised several flags for us, and we took actions to mitigate this. It is possible that the raw, unweighted sample is under representing demographic groups that would be most likely to lose insurance. For instance, it is possible that the raw, unweighted sample is under representing demographic groups that would be most likely to loss insurance, and therefore deflating this metric. We have opted to utilizing replicate weights available by Census within our model fitting process to try and account for this discrepancy.

Secondly, with a low rate of insurance loss within our sample, we understand that there may be a high likelihood of overfitting or bias our model. We will assess our model against several error metrics, to interpret the utility of our model and within the model selection phase. This is expanded on within our model development section further.
...
Food security can be an important predictor of uninsurance. One study found that compared to people who are food secure, individuals with food insecurity "postpone needed medical care and medications more often, use more emergency and inpatient care, and have higher care expenditures by as much as 121%.[^10]

[^10]: Sonik, 2019

The below visualization shows the distribution of food security in our sample. We found that the large majority of respondents identified as having high food security. A simplistic inference could be that any "risks" associated with low food security and uninsurance were less present in our sample population.
...
Given the focus on health insurance, we were also interested to see the distribution of coverage types in this sample. We found that around 2/3rd of the insured population in the US had private health insurance through an employer. Given how many in our sample rely on their job for insurance, We could infer or hypothesize that employment variables within the data set may play heavily into the predictors of "insurance_gap".
...
In addition to exploring several variable of interest, we sought to understand missingness within our sample.

The U.S. Census Bureau utilized and published documentation around their own data imputation sources. Given the extensive expertise of the U.S. Census Bureau within the SIPP data, we relied heavily on their work to mitigate systematically missing data that would have tampered with our analysis results. Census imputation protocols were as follows, based on the assumption that data are missing at random within subgroups of the population. The imputation methods used are:
...
After exploratory analysis, we made several decisions to refine our data so that it would be in the best position to create a robust model. Namely, the SIPP dataset comes with a large, computationally-costly, set of variables. We focused on taking out variables that would be either least likelihood to improve predictions or most likely to cause overfitting or bias.

### Identifying Zero Variance Predictors

First, we focused on removing variables that had almost zero variance, which would limit their utility in predicting "insurance_gap" and add substantial computational costs. We compiled a list of zero variance columns through the code below:
...
After identifying near-zero variance columns, we sought to remove predictors that were either perfectly correlated with insurance_gap or that would likely lead to overfitting or bias of our predictions (e.g., in-depth questions around specific public or private health insurance, uninsurance, etc). We utilized logical reasoning to sift through the hundreds of health insurance or employment questions that we believed would corrupt our model.
...
#commenting out because of lengthy result
...
#### Removing Other Variable at Risk of Overfitting or Bias
...
Below, you may find the code that was utilized to prepare our final data set, removing zero variance variables and highly correlated variables:
...
The U.S. Census Bureau provides two types of weights to help with analysis: replicate weights and longitudinal weights. Longitudinal weights compensate for panel attrition and post-stratification adjustments to make weighted sample totals confirm to population totals for key variables. Since we are only using data for a single month, we attempted to use person-level replicate weights, which allow a single sample to simulate multiple samples, thus generating more standard errors that maintain the normal properties of standard errors while retaining the complex survey design.

```{r, cache=TRUE, cache.id="joining_weights"}
#| message: false
#| warning: false

#read in the weighting files from SIPP website
rw_2018 <- fread("data/rw2018.csv", sep = "|", showProgress = TRUE) %>%
  select(SSUID, PNUM, repwgt0, monthcode) %>%
  mutate(unique_id = paste(SSUID,PNUM))
          

#filter by January weights
...
Please note: we had several options for when to apply weights to our data set. We found mixed research on when to apply weighting to our data set (before preprocessing, during preprocessing, and model fitting). Ultimately, we decided to apply weights during model fitting for several reasons. Our understanding is that weighting during model fitting cuts down on computational costs. This helped us mitigate challenges we consistently experienced with computer processing bandwidth.

## Model Development and Implementation

For our supervised machine learning model, we decided to develop a random forest model. This algorithm was selected for several reasons. First, given the level of nuance that goes into obtaining or losing insurance (e.g., Medicaid varying from state to state), we felt that the model could capture underlying patterns in our data with a high level of accuracy. Additionally, our team experienced a number of hurdles when running computationally heavy code. While a random forest can be costly, it can be highly effective with relatively little hyper parameter tuning, and provides options for parallelization.
...
Given the advantages of a random forest model, we kept feature engineering to a minimum, namely removing nominal/categorical variables that had little predictive advantage and conducing basic data imputation.
...
In terms of hyperparameter tuning, we tested only the trees parameter given computational costs, but assigned min_n and mtry based on best practices for the amount of variables and observations within each fold.
...
resamples <- workflow %>%
  tune_grid(
    ...
    #weight by monthly replication variable
    weights = REPWGT0
  )
...
We decided to prioritize two error metrics in selecting our best mode: precision and recall. We did this for several reasons. The first is that, given that our outcome "event" only occurred around 1% of the time, we wanted to prioritize the effectiveness of identified positive instances through assessing the proportion of correctly predicted positive instances, as well as the proportion of positive instances that were predicted out of all true positives.

Additionally, we felt that these metrics had a more relevant policy application. Focusing on the effectiveness of the true positive rates ensures that those that actually are most likely to lose insurance are identified by the model. In the event that someone is identified as falsely negative, it is possible that that efforts for outreach to mitigate uninsurance would be 1) inefficient in terms of costs, and 2) harmful in not reaching those that are most at at risk of harm due to uninsurance.

In terms of results, the initial model testing generated two models with 100 and 500 trees. Both models reported a recall of 1 and marginally different precision metrics, respectively. Given that recall was equal, we utilized precision as the metric to select our best model.

### Analyzing Results and Evaluating Final Model

Using our finally fitted model, we applied the best model to our testing data:
...
In order to assess the true performance of the model against the testing data, we constructed a confusion matrix.
...
The model correctly identified all true negatives. However, it did not identify any true positives, and had 194 false negatives. Unfortunately, this would seem to confirm our concerns that there is an imbalance in classes within our data set. We offer some methods of improving the prediction of insurance loss in future analysis.

### Variable Importance

Although our model results were mixed, we sought to investigate the variable importance that the model used to predict insurance loss.
...
The month when a person began a job is the most important predictor. We hypothesize this may be because private insurance is closely related to employment. Specifically, it could be possible that individuals working full time within a certain period of the year may be seasonally employed and could lose insurance after their assignment has concluded. Several of the next most important predictors are also variables related to employment, further confirming our earlier predictions of the importance of employment to health insurance.

Age, income, geography and race are also important predictors. Despite mixed results of our model, these results would seem to remain consistent with existing literature. For children and the elderly, public insurance options are readily available (Medicaid and Medicare, respectively). Studies have shown that the uninsured population is predominantly made up of middle-aged working adults, so we are not surprised that age is an important predictor.[^11] Income is also an important predictor. A major barrier to coverage is cost; even with insurance, costs associated with insurance (premiums, co-pays, deductibles) can be a barrier for lower-income individuals that may make too much to qualify for Medicaid. Race is also not a surprising inclusion; the majority of the uninsured population is comprised of other races and ethnic groups.[^12]
...
This model, if improved, could help inform policymakers at the state, federal, and local levels where to focus fiscal and legislative priorities to improve health insurance coverage and the overall public health of communities. By being able to predict a citizen's loss of coverage, and understanding what comprises the uninsured population, programs could target the root causes of health insurance loss, like job loss, health inequities, and poverty. While stopgaps do exist for people who lose employment, and thus insurance, the coverage is expensive and poor in many places. Improving insurance coverage, and anticipating those most likely to lose insurance can ensure intelligent allocation of resources for unemployed populations could reduce overall healthcare spending and improve the health and well-being of communities.
...
Ultimately, these factors, among others, should be considered prior to expanding on our model or utilizing to understanding broader, generalization predictions. At this point in time, we would not recommend the utilization of our model to inform policy decisions.
...
Berchick, E. (2018). *Who Are the Uninsured?* U.S. Census Bureau. <https://www.census.gov/library/stories/2018/09/who-are-the-uninsured.html>
...
Lukens, G. & Sharer, B. (2021). *Closing Medicaid Coverage Gap Would Help Diverse Group and Narrow Racial Disparities*. The Center on Budget and Policy Priorities (CBPP). <https://www.cbpp.org/research/health/closing-medicaid-coverage-gap-would-help-diverse-group-and-narrow-racial#_ftn6>

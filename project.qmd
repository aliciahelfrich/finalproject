...
\<\<\<\<\<\<\< HEAD Given the negative consequences of insurances loss and the costs associated with re-enrolling individuals in insurance, we sought to develop a predictive model that would assist government in targeting and anticipating individuals that may be most as risk of losing their insurance. If effective, uch a predictive model can help state and federal policymakers and other interested stakeholders target outreach and enrollment activities.
...
\<\<\<\<\<\<\< HEAD Given how low this metric appeared, decided to conduct a nonparametric binomial test to understand whether this estimate was statistically different than the insurance loss rate we would expect for the US population. Based on findings from this exercise, we adjusted our analysis accordingly.
...
We were able to find that the [The Center on Budget and Policy Priorities](https://www.cbpp.org/research/health/closing-medicaid-coverage-gap-would-help-diverse-group-and-narrow-racial#_ftn6) reports 13% of middle-aged individuals that had Medicaid in January had experienced some kind of a gap in coverage throughout the year. Using this as an anchor, we would expect that this proportion would be lower for the general population. Those on private insurance, which make up most cases (as we show below), are more likely to be in sector that is at lower risk of turnover(and thus loss of private insurance), or be more likely to have resources to purchase personal insurance in the event of losing their job. We decided to assume, based on limited data, that the true probability of losing insurance for our population of interest would be 10%.
...
In addition to exploring several variable of interest, we sought to understand missingness within our sample. Luckily, the U.S. Census Bureau utilized and published documentation around their own data imputation sources. Given the extensive expertise of the U.S. Census Bureau within the SIPP data, we relied heavily on their work to mitigate systematically missing data that would have tampered with our analysis results. Census imputation protocols were as follows, based on the assumption that data are missing at random within subgroups of the population. The imputation methods used are:
...
After exploratory analysis, we made several decisions to refine our data so that it would be best positions to create a robust model. Namely, the SIPP dataset comes with a large, computationally-costly, set of variables. We focused on taking out variables that would be either least likelihood to improve predictions or most likely to cause overfitting or bias.

### Identifying Zero Variance Predictors

First, we focused on removing variables that had almost zero variance, which would limit their utility in predicting "insurance_gap" and add substiantial computational costs. We compiled a list of zero variance columns through the code below:
...
Please note: although we are, in the case below, were investigating the relationship between two categorical variables (versus continuous), we found the exercise useful for exploratory purposes, with lower computational costs of other tests of association.
...
There were 8 variables that were found to be perfectly associated with insurance_gap, namely dealing with employment metrics. Specifically, many of the variables, removed in the code below, were related to employment/unemployment spells.
...
Below, you may find the code that was utilized to prepare our final dataset, removing zero variance variables and highly correlated variables:
...
As previously discussed, based on exploratory analysis, as well as a through review of SIPP documentation, we decided it would be prudent to incorporate survey weights into our analysis. The SIPP offers CSV files matched to unique participants that provides weights for each observation - both on a month-by-month or a yearly basis. We decided to utilize weights associated with January estimates, given that our data is filtered to include relevant information for participants in January.
...
Please note: we had several options for when to apply weights to our dataset. We found mixed research on when to apply weighting to our dataset (before preprocessing, during preprocessing, and model fitting). Ultimately, decided to apply weights during model fitting for several reasons. First, our understanding is that weighting during model fitting cuts down on computational costs. This helped us mitigate challenges we consistently experienced with computer processing bandwidth.
...
Using out finally fitted model, we applied this model to our testing data:
